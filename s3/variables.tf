...
variable "buckets" {
  ...
  validation {
    condition     = alltrue([for bucket in var.buckets : bucket.advanced.acl == "" || contains(["private", "public-read", "public-read-write", "authenticated-read"], bucket.advanced.acl)])
    error_message = "Bucket ACL must be one of private, public-read, public-read-write, or authenticated-read when specified."
  }
  validation {
    condition     = alltrue([for bucket in var.buckets : !bucket.advanced.replication.enable || length(trim(bucket.advanced.replication.role_arn)) > 0])
    error_message = "advanced.replication.role_arn must be set for buckets with advanced.replication.enable = true."
  }
}
