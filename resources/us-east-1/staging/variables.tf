...
variable "nat_gateway_count" {
  description = "Number of NAT Gateways (1 for staging, 3 for production HA)"
  type        = number
  default     = 1
}
...
variable "eks_public_access_cidrs" {
  description = "CIDRs allowed to reach the EKS public API endpoint (restrict to VPN/office range)"
  type        = list(string)
}
